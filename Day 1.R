...
data_sub <- data[sample(nrow(data), 10000), ]
...
x4_vars <- c("count_p_1","max_p_2","p_4","p_5","sum_p_6","count_p_7")
#"count_p_8","count_p_9","count_p_10","count_p_11","count_p_12",
#"count_p_13","len_p_14","count_p_15","count_p_19","count_p_20")


# Create a formula specifying the variable to one-hot encode
formula <- as.formula(~ campaign)

# Use dummyVars to create one-hot encoded columns
one_hot_encoded <- dummyVars(formula, data = data)

# Apply the one-hot encoding to your data
encoded_data <- data.frame(predict(one_hot_encoded, newdata = data))

# Assuming 'data' contains your dataframe with one-hot encoded columns
means <- colMeans(encoded_data)

condition <- means < 0.04


# Create a new column named 'other' with all zeros
encoded_data$other <- 0

# Sum values of dummy variables meeting the condition and assign to 'other' column
encoded_data$other <- rowSums(encoded_data[, condition])

# Remove individual dummy variables meeting the condition
encoded_data2 <- encoded_data[, !condition]
encoded_data_final <- cbind(encoded_data2, encoded_data$other)

# Remove the original 'media_source' column if needed
data <- data[, -which(names(data) == "media_source")]

# Assuming you have one-hot encoded 'media_source' and stored it in 'encoded_data'
data3 <- cbind(data2, encoded_data2)

#Subset new data set
data_sub3 <- data3[sample(nrow(data), 10000), ]


# Create new variable sets that include the one-hot encoded variables
x3_vars <- c("d14_spend","d7_spend","d3_spend","d1_spend",
             "count_p_1","max_p_2","sum_p_6","count_p_7","media_sourcems_0","media_sourcems_1","media_sourcems_3","media_sourcems_6","media_sourcems_8")

x4_vars <- c("media_sourcems_0","media_sourcems_1","media_sourcems_3","media_sourcems_6","media_sourcems_8")

x5_vars <- c("campaignc_0","campaignc_10","campaignc_19","campaignc_5","campaignc_6","campaignc_7", "media_sourcems_0","media_sourcems_1","media_sourcems_3","media_sourcems_6","media_sourcems_8")

# train models with these different models, xX_vars was interchanged to not clutter the code
rf_x5 <- train(as.formula(paste("d30_spend", paste(x5_vars, collapse = " + "), sep = " ~ ")), 
               data = data_sub3, 
               method = "ranger",
               #method = "rpart",
               trControl = cv_fold,
...
rf_x3 <- train(as.formula(paste("y", paste(x3_vars, collapse = " + "), sep = " ~ ")), 
               data = data_sub2, 
               method = "ranger",
               #method = "rpart",
               trControl = cv_fold,
